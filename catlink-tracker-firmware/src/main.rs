...
fn gnss_on(dp: &mut DevicePeripherals) {
    ...
    dp.GPIOB.bsrr().write(|w| w.bs3().set_bit());
...
fn gnss_off(dp: &mut DevicePeripherals) {
    dp.GPIOB.bsrr().write(|w| w.br3().set_bit());
...
fn main() -> ! {
    ...
    // B4: DIO1
    // B3: EN | B6: TX | B7: RX
    dp.GPIOB.moder().write(|w| w.moder3().output().moder4().input().moder6().alternate().moder7().alternate());
    dp.GPIOB.otyper().write(|w| w.ot3().push_pull());
    dp.GPIOB.bsrr().write(|w| w.br3().set_bit());
    ...
    dp.EXTI.rtsr1().write(|w| w.tr4().set_bit().tr15().set_bit().tr20().set_bit());
    dp.EXTI.imr1().write(|w| w.mr4().set_bit().mr15().set_bit().mr20().set_bit());
